...
public class KEGG2yGraph extends AbstractKEGGtranslator<Graph2D> {
  ...
  public static NodeRealizer setupGroupNode(NodeLabel nl, String changeCaption) {
    ...
    if (nl!=null) {
      ...
      nl.setBackgroundColor(new Color(0.8f, 0.8f, 0.8f, 0.5f));
      nl.setFontSize(10);
  ...
  protected Graph2D translateWithoutPreprocessing(Pathway p) {
    ...
    for (int i=0; i<p.getEntries().size(); i++) {
      ...
      if (skipCompounds && e.getType().equals(EntryType.compound)) {
        continue;
      }
      ...
      if (name.toLowerCase().startsWith("path:") || e.getType().equals(EntryType.map)) {
      ...
      if (n!=null) {
        ...
        for (String ko_id:e.getName().split(" ")) {
          ...
          if (ko_id.equalsIgnoreCase("undefined") || ko_id.equalsIgnoreCase("null") || e.hasComponents()) {
            continue;
          }
        ...
        if (isPathwayReference) {
          PWReferenceNodeTexts.add(graph.getRealizer(n).getLabelText());
        }
        if (e.hasReaction()) {
    ...
    for (int i=0; i<p.getRelations().size(); i++) {
      if (showProgressForRelations) {
        progress.DisplayBar("Relation " + (i+1) + "/" + p.getRelations().size());
      }
        ...
        for (int stI=0; stI<r.getSubtypes().size(); stI++) {
          ...
          if (st.getName().trim().equalsIgnoreCase("compound") && st.getValue()!=null && Utils.isNumber(st.getValue(),true)) {
            ...
            if (nOne.getEdgeTo(nTwo)==null) {
              ...
              if (st!=null)
              {
                interactionDescription.set(myEdge, st.getName()); // => Subtype Name // compound, hidden compound, activation, inhibition, expression, repression, indirect effect, state change, binding/association, dissociation, missing interaction, phosphorylation, dephosphorylation, glycosylation, ubiquitination, methylation
    ...
    if (groupNodesWithSameEdges) {
      ...
      for (int i=0; i<myNodes.length-1; i++) {
        ...
        if (hm.isGroupNode(myNodes[i]) || hm.getParentNode(myNodes[i])!=null || !hm.isNormalNode(myNodes[i]) ||
            myNodes[i].edges().size()<1 || (type!=null && type.equals("reaction")) ) {
          continue;
        }
        
        for (int j=i+1; j<myNodes.length; j++) {
          if (hm.isGroupNode(myNodes[j]) || hm.getParentNode(myNodes[j])!=null || !hm.isNormalNode(myNodes[j])) {
            continue;
          }
