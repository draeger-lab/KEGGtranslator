...
ALTERNATIVE: Generating systems biology models from KEGG pathways}
...
The KEGG PATHWAY database provides a valuable resource for initial modeling approaches of specific biological networks \cite{Kanehisa2000}. The database contains pathway maps for a multitude of different organisms and most provided information is cross-linked with other KEGG databases. Since many years, this database is one of the most important sources for building initial structural models of various pathways \cite{Bauer-Mehren2009,Oberhardt2009}. All pathway information is stored in KGML formatted xml-files, which are barely supported by other applications. In systems biology, two wide-spread formats for modeling and exchanging pathways are the SBML and BioPAX formats \cite{Finney2003,Demir2010}. These formats can be used with graphical modeling applications, like CellDesigner \cite{Funahashi2008}, annotated with kinetics (TODO: squeezer subliminal), used for flux balance analysis (TODO fasimu), etc. (TODO anwendungen für BioPAX finden). Therefore, converters exist that perform mostly basic conversions from KGML to those formats \cite{KEGGconverter,KEGG2SBML}. The drawback of many of those conversions is, that even for creating initial models a basic translation of a KGML document to, e.g., an SBML document is not sufficient.
...
This is an important step, because simulations on real data or simple experimental data visualization requires unique identifiers to map the experimental data on the pathway data structure. If models provide a simple data structure with labels, but no reference identifiers, they are hardly usable in conjunction with experimental data.
...
\subsection{KEGG to SBML-qual}


\subsection{KGML entries that are reactions}
...
%% TODO: SubType compound bei relations ist auch ein spezialfall!
%% - Glycan 2 Compound (same as) provides formula, mass, etc.
