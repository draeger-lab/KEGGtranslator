...
All pathway entries can be connected to the other KEGG databases, like KEGG ENZYME, KEGG GENE, etc.
Subsequently, the pathways contain an enormous amount of information and can be used as foundations for initial structural modeling approaches.
But the KGML-formatted KEGG pathways are primarily thought for visualization and thus, the direct conversion of them into systems biology models would produce incomplete and erroneous models.

Here, we propose a method for preprocessing and converting KEGG pathways into initial structural models in the standardized community pathway formats SBML (level 2 and 3) and BioPAX (level 2 and 3). 
The preprocessing step comprises KGML-content fixing, atom balance check of reactions, and augmenting the pathway information content with information from various other databases, such as Entrez Gene, Omim, ChEBI, etc.
Furthermore, we showed that the conversion from KEGG into BioPAX is very handy, while the translation into SBML can only be done without loss of information using SBML level~3 including the recently proposed qualitative models and groups extensions.
...
Next to biochemical reactions, BioPAX also supports other kinds of relationships between physical entities. 
BioPAX distinguishes between interactions for which one can specify a source and a target and interactions describing a pool of interacting components.
For instance to express KEGG relations, which have no associated chemical equation, but structural information like ``A activates B", the first BioPAX interaction type can be used. 
The second interaction type is especially useful for cases in which information is missing or no direction is available. 
For example, a relation of type binding with two participants, exclusively, allows us to express ``A binds B". In this example no other conclusion can be drawn from such a relation.
Therefore, all relations from which no direction can be inferred are converted to a BioPAX physicalInteraction in BioPAX level 2 or to a MolecularInteraction in BioPAX level 3 (the level 2 physicalInteraction has been replaced by MolecularInteraction in level 3). All relations, from which a direction can be inferred, are converted to a BioPAX conversion.
